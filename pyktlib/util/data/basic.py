...
def write_kt_data(data, data_path):
    ...
    id_keys_ = set(id_keys).intersection({"user_id", "school_id", "premium_pupil", "gender", "seq_len", "campus",
                                          "dataset_type", "order"})
    seq_keys_ = set(seq_keys).intersection({"question_seq", "concept_seq", "correctness_seq", "time_seq", "use_time_seq",
...
def read_kt_data(data_path):
    with open(data_path, "r") as f:
        ...
        for i, line_str in enumerate(all_lines):
            ...
            if current_key in ["time_factor_seq", "hint_factor_seq", "attempt_factor_seq", "correctness_float_seq"]:
